...
import { importObj } from "../world.js";
...
export async function convertXML(world, filename) {
    ...
    world.data.assets.models["roblox_cylinder"] = await importObj("/kodiak/3rdparty/roblox/shapes/cylinder.obj")
    world.data.assets.models["roblox_wedge"] = await importObj("/kodiak/3rdparty/roblox/shapes/wedge.obj")
    world.data.assets.models["roblox_cornerwedge"] = await importObj("/kodiak/3rdparty/roblox/shapes/cornerwedge.obj")
    ...
    for (let part of parts) {
        ...
        if (shape_code == "0") {
            ...
            model = "roblox_cylinder"
        } else if (shape_code == "3") {
            model = "roblox_wedge"
        } else if (shape_code == "4") {
            model = "roblox_cornerwedge"
        ...
        // ========
        let new_part_id = world.newPart();
        let new_part = world.getPartByID(new_part_id);

        new_part.position[0] = transform.position[0]*scale_factor;
        new_part.position[1] = transform.position[1]*scale_factor;
        new_part.position[2] = transform.position[2]*scale_factor;

        new_part.scale[0] = transform.scale[0]*scale_factor;
        new_part.scale[1] = transform.scale[1]*scale_factor;
        new_part.scale[2] = transform.scale[2]*scale_factor;

        new_part.rotation[0] = transform.rotation[0];
        new_part.rotation[1] = transform.rotation[1];
        new_part.rotation[2] = transform.rotation[2];

        let kodiak_material_name = `Roblox ${material_name} (${r},${g},${b})`;

        if (! (kodiak_material_name in world.data.assets.materials)) {
            let part_mat = world.newMaterial(kodiak_material_name);
            part_mat.textures.diffuse = `../3rdparty/roblox/materials/${material_name}/color.png`;
            part_mat.tint = [r, g, b];
            part_mat.stretch = false;
            part_mat.scale = .2/scale_factor
        }

        new_part.material = kodiak_material_name;
        new_part.model = model

        world.manifest(new_part_id);
    }
}